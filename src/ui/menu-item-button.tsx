...
export function MenuItemButton({ onClick, icon, label, disabled = false }: MenuItemButtonProps) {
  return (
    <MenuItem disabled={disabled}>
      <ButtonBase
        onClick={onClick}
        className="flex w-full items-center gap-3 rounded-lg px-3 py-2 transition-colors data-disabled:opacity-50 data-focus:bg-slate-100 dark:data-focus:bg-slate-700"
        tabIndex={-1}
        type="button"
        disabled={disabled}
      >
